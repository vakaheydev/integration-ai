...
public class SwaggerServiceDocument {
    ...
    public void deleteAllByUserId(String userId) {
        List<SwaggerDocument> docs = documentStorageService.findByUserId(userId);
        for (SwaggerDocument doc : docs) {
            var cache = cacheManager.getCache(CacheConfig.SWAGGER_DOCUMENTS_CACHE);
            if (cache != null) cache.evict(doc.getId());
            vectorStorageService.deleteById(doc.getId());
        }
        documentStorageService.deleteAllByUserId(userId);
        log.debug("Deleted all documents for userId={}", userId);
    }

    /**
     * Извлекает блок info и servers из OpenAPI-документа в читаемый текст.
     * Используется как префикс к documentSummary чтобы ИИ знал базовую инфу об API.
     */
    public String resolveSwaggerInfoSummary(String swaggerContent) {
        OpenAPI openAPI;
        try {
            openAPI = parseOpenAPI(swaggerContent);
        } catch (Exception e) {
            log.warn("Failed to parse OpenAPI for info summary: {}", e.getMessage());
            return "";
        }
        ...
        if (openAPI.getServers() != null && !openAPI.getServers().isEmpty()) {
            ...
            for (var server : openAPI.getServers()) {
                sb.append("  - ").append(server.getUrl());
                if (server.getDescription() != null) sb.append(" (").append(server.getDescription()).append(")");
                sb.append("\n");
            }
        }
        ...
        if (openAPI.getSecurity() != null && !openAPI.getSecurity().isEmpty()) {
            sb.append("GLOBAL SECURITY:\n");
            for (var req : openAPI.getSecurity()) {
                req.forEach((scheme, scopes) -> {
                    sb.append("  - ").append(scheme);
                    if (scopes != null && !scopes.isEmpty()) sb.append(" [").append(String.join(", ", scopes)).append("]");
                    sb.append("\n");
                });
            }
        }
                    ...
                    operation.getSecurity().forEach(req ->
                        req.forEach((scheme, scopes) -> {
                            ...
                            if (scopes != null && !scopes.isEmpty())
                                sb.append("(").append(String.join(",", scopes)).append(")");
                        })
                    );
                    sb.append("]");
                } else if (openAPI.getSecurity() != null && !openAPI.getSecurity().isEmpty()) {
                    // наследуем глобальный security если у операции нет своего
                    sb.append(" [security: inherited-from-global]");
                }
    ...
    public Optional<String> extractSchema(String swaggerContent, String schemaPath) {
        ...
        // Если путь указывает на securitySchemes — извлекаем оттуда
        if (schemaPath != null && schemaPath.contains("securitySchemes")) {
            return extractSecurityScheme(openAPI, resolveSchemaName(schemaPath));
        }

        // Стандартная логика — ищем в components/schemas
        ...
        // Если не нашли в schemas — пробуем в securitySchemes как fallback
        if (schema == null) {
            Optional<String> securityResult = extractSecurityScheme(openAPI, schemaName);
            if (securityResult.isPresent()) {
                return securityResult;
            }
            log.warn("Schema '{}' not found in components/schemas or securitySchemes", schemaName);
            return Optional.empty();
        }
    ...
    private Optional<String> extractSecurityScheme(OpenAPI openAPI, String schemeName) {
        ...
        StringBuilder sb = new StringBuilder();
        sb.append("SecurityScheme: ").append(schemeName).append("\n");
        if (scheme.getType() != null) sb.append("  type: ").append(scheme.getType()).append("\n");
        if (scheme.getScheme() != null) sb.append("  scheme: ").append(scheme.getScheme()).append("\n");
        if (scheme.getBearerFormat() != null) sb.append("  bearerFormat: ").append(scheme.getBearerFormat()).append("\n");
        if (scheme.getIn() != null) sb.append("  in: ").append(scheme.getIn()).append("\n");
        if (scheme.getName() != null) sb.append("  name: ").append(scheme.getName()).append("\n");
        if (scheme.getDescription() != null) sb.append("  description: ").append(scheme.getDescription()).append("\n");
        if (scheme.getOpenIdConnectUrl() != null) sb.append("  openIdConnectUrl: ").append(scheme.getOpenIdConnectUrl()).append("\n");
        if (scheme.getFlows() != null) {
            sb.append("  flows:\n");
            var flows = scheme.getFlows();
            if (flows.getImplicit() != null) sb.append("    implicit: authorizationUrl=").append(flows.getImplicit().getAuthorizationUrl()).append("\n");
            if (flows.getPassword() != null) sb.append("    password: tokenUrl=").append(flows.getPassword().getTokenUrl()).append("\n");
            if (flows.getClientCredentials() != null) sb.append("    clientCredentials: tokenUrl=").append(flows.getClientCredentials().getTokenUrl()).append("\n");
            if (flows.getAuthorizationCode() != null) sb.append("    authorizationCode: authorizationUrl=").append(flows.getAuthorizationCode().getAuthorizationUrl()).append(", tokenUrl=").append(flows.getAuthorizationCode().getTokenUrl()).append("\n");
        }
        return Optional.of(sb.toString());
    }
